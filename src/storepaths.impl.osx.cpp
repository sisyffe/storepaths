...
static inline bool
getSpecificFolder(SizedStream &stream,
...
DEFINE_GET_FOLDER_FUNCTION(getCacheFolder,
...
std::pair<std::string, PathInfo>
getOsxConfigFile(const std::string &appName,
                 const std::optional<std::string> &fileName) {
  UNUSED(appName) // OSX implementation does not use this but others do so need
                  // the dignature to be the same
